...
.PHONY: setup run stop status autostart autostart-off init commit push clear clean uninstall

setup:
	python3 -m venv $(VENV)
	$(PYTHON) -m pip install -r requirements.txt
	@cp app/resources/config_example.yaml config.yaml
	@echo "created config.yaml from app/resources/config_example.yaml"
	@if [ ! -f .env ]; then printf '%s\n' 'GITHUB_TOKEN=' > .env; echo "created .env - put your token in it before running"; fi
	@mkdir -p $(BIN_DIR)
	@printf '%s\n' \
		'#!/usr/bin/env bash' \
		'case "$$1" in' \
		...
		'  stop|status|autostart|autostart-off|clear|clean)' \
...
# Единый запуск:
# make run      -> обычный запуск в консоли
# make run q=1  -> тихий запуск в фоне
run:
	@if [ "$(q)" = "1" ] || [ "$(quiet)" = "1" ]; then \
...
# Автозапуск: создаёт стандартизированный .desktop файл в ~/.config/autostart/
autostart:
...
# Отключение автозапуска: удаляет .desktop файл
autostart-off:
	@rm -f $(DESKTOP_FILE)
